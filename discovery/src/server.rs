...
impl RequestHandler {
    ...
    fn handle_add_user(&self, params: &Params<'_>) -> Response<hyper::Body> {
        ...
        let checksum_key = {
            let mut h = Hmac::<Sha1>::new_varkey(base_key).expect("HMAC can take key of any size");
            h.update(b"checksum");
        ...
        let mut h = Hmac::<Sha1>::new_varkey(&checksum_key).expect("HMAC can take key of any size");
        h.update(encrypted);
